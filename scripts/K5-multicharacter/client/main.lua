...
CreateThread(function()
	while true do
		...
		if NetworkIsSessionStarted() then
			TriggerEvent('K5-multicharacter:client:chooseChar')
			return
		end
	end
end)
...
local function openCharMenu(bool)
    QBCore.Functions.TriggerCallback("K5-multicharacter:server:GetNumberOfCharacters", function(result)
        local translations = {}
...
RegisterNetEvent('K5-multicharacter:client:closeNUIdefault', function() -- This event is only for no starting apartments
...
RegisterNetEvent('K5-multicharacter:client:closeNUI', function()
    DeleteEntity(charPed)
    SetNuiFocus(false, false)
end)

RegisterNetEvent('K5-multicharacter:client:chooseChar', function()
    SetNuiFocus(false, false)
    DoScreenFadeOut(10)
    Wait(1000)
...
RegisterNUICallback('disconnectButton', function(_, cb)
    ...
    DeleteEntity(charPed)
    TriggerServerEvent('K5-multicharacter:server:disconnect')
    cb("ok")
end)
...
RegisterNUICallback('selectCharacter', function(data, cb)
    ...
    DoScreenFadeOut(10)
    TriggerServerEvent('K5-multicharacter:server:loadUserData', cData)
    openCharMenu(false)
    SetEntityAsMissionEntity(charPed, true, true)
    DeleteEntity(charPed)
    cb("ok")
...
RegisterNUICallback('cDataPed', function(nData, cb)
    ...
    DeleteEntity(charPed)
    if cData ~= nil then
        QBCore.Functions.TriggerCallback('K5-multicharacter:server:getSkin', function(model, data)
...
RegisterNUICallback('setupCharacters', function(_, cb)
    QBCore.Functions.TriggerCallback("K5-multicharacter:server:setupCharacters", function(result)
        SendNUIMessage({
...
RegisterNUICallback('createNewCharacter', function(data, cb)
    ...
    TriggerServerEvent('K5-multicharacter:server:createCharacter', cData)
    Wait(500)
    cb("ok")
end)

RegisterNUICallback('removeCharacter', function(data, cb)
    TriggerServerEvent('K5-multicharacter:server:deleteCharacter', data.citizenid)
    DeletePed(charPed)
    TriggerEvent('K5-multicharacter:client:chooseChar')
    cb("ok")
end)